...
mod tests {
    ...
    fn test_parameter_parm_wrapped_on_pack() {
        ...
        let stored_crc = u32::from_le_bytes(
            param_data[8 + content.len()..12 + content.len()].try_into().unwrap(),
        );
        assert_eq!(stored_crc, 0x2c36fd8a, "PARM CRC must match Rockchip rkcrc32");
    }

    #[test]
    fn test_pre_wrapped_parameter_is_not_wrapped_again() {
        let temp_dir = TempDir::new().unwrap();
        let content = b"CMDLINE:console=ttyFIQ0,1500000\n";
        let wrapped_size = content.len() + 12;
        let mut pre_wrapped = Vec::with_capacity(wrapped_size);
        pre_wrapped.extend_from_slice(b"PARM");
        pre_wrapped.extend_from_slice(&(content.len() as u32).to_le_bytes());
        pre_wrapped.extend_from_slice(content);
        // A structurally valid pre-wrapped blob must be preserved byte for
        // byte even when its stored CRC came from a legacy external script.
        pre_wrapped.extend_from_slice(&0xc89c2297u32.to_le_bytes());

        let metadata = format!(
            "parameter,parameter.txt,0x00000000,0x00000000,0x00000800,{:#010x},{:#010x}\n",
            wrapped_size.div_ceil(2048),
            wrapped_size,
        );

        fs::write(temp_dir.path().join("parameter.txt"), &pre_wrapped).unwrap();
        fs::write(temp_dir.path().join("package-file"), "parameter parameter.txt\n").unwrap();
        fs::write(temp_dir.path().join("partition-metadata.txt"), metadata).unwrap();

        let output_image = temp_dir.path().join("output.rkaf");
        pack_rkaf(
        ...
        let output = fs::read(&output_image).unwrap();
        let header = UpdateHeader::decode(&output[..UPDATE_HEADER_SIZE]).unwrap();

        assert_eq!(header.parts[0].part_byte_count as usize, wrapped_size);
        assert_eq!(&output[2048..2048 + wrapped_size], pre_wrapped);
    }
